...
describe("Party Call Service", function() {
  ...
  it("Output the election results in CSV format", done => {
    ...
    let filepath =
      "/usr/src/app/test/services/party-call/Bronx Party Call 2018 - Sheet1.csv";
  ...
  it("can read different field configurations for formatted csv", done => {
    let filepath =
      "/usr/src/app/test/services/party-call/Kings Democratic County Party Call - Sheet1.csv";
    const PartyCallService = app.service(app.get("apiPath") + "/party-call");
    ...
    PartyCallService.create({
      ...
      .then(partyCall => {
        ...
        assert.equal(
          partyCall.source,
          "Kings Democratic County Party Call - Sheet1.csv"
        );
        assert(Array.isArray(partyCall.positions));
  ...
  it("can read different field configurations for formatted csv", done => {
    let filepath =
      "/usr/src/app/test/services/party-call/KingsDem2020PartyCall-CONVERTED-GROOMED-V2.csv";
    const PartyCallService = app.service(app.get("apiPath") + "/party-call");
    ...
    PartyCallService.create({
      ...
      .then(partyCall => {
        ...
        assert.equal(
          partyCall.source,
          "KingsDem2020PartyCall-CONVERTED-GROOMED-V2.csv"
        );
        assert(Array.isArray(partyCall.positions));
        assert.equal(partyCall.positions.length, 5487);
        assert.equal(partyCall.positions[0].party, "Democratic");
        assert.equal(partyCall.positions[0].office, "Male County Committee");
        assert.equal(partyCall.positions[1].office, "Female County Committee");

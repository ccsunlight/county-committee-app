/**
 *
 * This service extracts members from documents.
 *
...
exports.getNonFoundMembersRecursive = function(
  ccMembers,
  unfoundMembers,
  callback
) {
  ...
  countyCommitteeModel.find(
    ...
    function(err, foundMembers) {
      ...
      if (ccMembers.length === 0) {
        ...
        this.insertMembersIntoVacantPositionsRecursive(
          unfoundMembers,
          anomalyMembers,
...
this.insertMembersIntoVacantPositionsRecursive = function(
  members,
  anomalyMembers,
  callback
) {
  ...
  csv
    ...
    .on("data", function(data) {
      if (data[0] == "Total Applicable Ballots") {
        ...
        var found = data[0].match(
          /County Committee \((\d+)\/(\d+)\) - (.+) County/
        );
